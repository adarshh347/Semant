...
## 12.8 Alexia — carousel sweep + split queue (2026-07-02/03, branch `feat/alexia-carousel-sweep`)
Two extension-only features (no backend changes); design + plan docs in `docs/plans/2026-07-0{2,3}-alexia-*`.
- **Carousel sweep ("Save all")**: hovering a photo inside a carousel shows a teal **Save all** button — pages through IG's own Next chevron collecting every photo slide (largest `srcset` candidate, dedup by CDN filename, video slides skipped), pages back to restore position, then opens the pick-grid (`renderPickGrid`, generalized from `renderFrameReview` with `{title,hint,tags,noun,igContext,sourceUrl}`). Detection = slide `li`→`ul` track + chevron in ≤5 ancestors (aria-label "Next"/"Go back" first, icon-only geometric fallback); guards: img must dominate its `li` (rejects the modal's prev/next-POST pager), ≥2 slide `li`s, `sweeping` reentrancy flag, video-aware dry counting (3 truly-dry steps stop; cap 24). Saves tagged `carousel`.
- **Split queue (videos)**: **Split → Save is now fire-and-forget** — `queueSplit()` snapshots author context (element-anchored `instagramContextForSave(video)` + `location.href`) and starts `captureFrames(job)` immediately in the background (parallel jobs, per-job canvas); the blocking `splitVideo()` is gone. Hidden-tab-safe: the per-frame wait is `seeked`+`setTimeout` (no rAF), so tab switches only slow capture. Scrolling far can kill the element → job finalizes **partial** with collected frames (`missed>=2` seek timeouts or `!isConnected`). A bottom-right **queue chip** (`.ss-queue-chip`, `⏳ N splitting · M frames`) opens a **queue view** in the panel: per-job sections (status, ✕ discard → `job.cancelled` stops the loop; `capturingVideos` registry blocks re-queue while winding down), live append-only frame grids (`job.dropped` Set holds deselections), one **"Save all (N frames)"** → `massSaveQueue` (reentrancy-guarded via `massSaving`) posts each frame via shared `postFrame()` with **its own job's** igContext/sourceUrl, tagged `video-frame`; saved jobs clear. Queue is in-memory per page (refresh clears).
- Both passed adversarial subagent review (2 rounds each); verified `node --check` only — **manual IG checklist still pending** (checklists in the plan docs).
