# Review — `sam3-fold-phrase-matrix/carved_figure/actuator_direct__face`

*carved_figure · 'face' · actuator_direct · mode `actuator_direct` · locked to `concept_segment` · expected `positive`*

> Matrix cell from the pre-registered suite 'sam3-fold-phrase-matrix': family 'availability_control' (role 'availability_control') against fixture 'carved_figure'. The phrase was frozen before any live call in this lane and may not be changed now.
...
| lock held | yes |
| cold / warm | warm |
| latency | 5820 ms (load 0 ms) |
| organ status | **ok** |
| instances | 2 |
| mask areas (px) | 10189, 29402 |
| max pairwise IoU | 0.3452 |
| all masks well-formed | yes |
| conversion | 2 instance(s) → 2 measured + 1 interpretive descriptor(s), 1 naming withheld, 0 dropped |
| two-status preserved | yes |
| invariants held | yes |

## Attribution

**organ_succeeded** — the organ measured 2 instance(s) of 'face'. Whether those instances ARE that concept is not established here
