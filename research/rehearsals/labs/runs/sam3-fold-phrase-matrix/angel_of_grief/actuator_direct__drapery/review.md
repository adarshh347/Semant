# Review — `sam3-fold-phrase-matrix/angel_of_grief/actuator_direct__drapery`

*angel_of_grief · 'drapery' · actuator_direct · mode `actuator_direct` · locked to `concept_segment` · expected `open`*

> Matrix cell from the pre-registered suite 'sam3-fold-phrase-matrix': family 'object_scope' (role 'object_scope') against fixture 'angel_of_grief'. The phrase was frozen before any live call in this lane and may not be changed now.
...
| lock held | yes |
| cold / warm | warm |
| latency | 6013 ms (load 0 ms) |
| organ status | **ok** |
| instances | 1 |
| mask areas (px) | 26811 |
| max pairwise IoU | — |
| all masks well-formed | yes |
| conversion | 1 instance(s) → 1 measured + 0 interpretive descriptor(s), 1 naming withheld, 0 dropped |
| two-status preserved | yes |
| invariants held | yes |

## Attribution

**organ_succeeded** — the organ measured 1 instance(s) of 'drapery'. Whether those instances ARE that concept is not established here
