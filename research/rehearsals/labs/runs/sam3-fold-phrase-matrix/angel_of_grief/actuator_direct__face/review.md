# Review — `sam3-fold-phrase-matrix/angel_of_grief/actuator_direct__face`

*angel_of_grief · 'face' · actuator_direct · mode `actuator_direct` · locked to `concept_segment` · expected `positive`*

> Matrix cell from the pre-registered suite 'sam3-fold-phrase-matrix': family 'availability_control' (role 'availability_control') against fixture 'angel_of_grief'. The phrase was frozen before any live call in this lane and may not be changed now.
...
| lock held | yes |
| cold / warm | warm |
| latency | 6123 ms (load 0 ms) |
| organ status | **ok** |
| instances | 5 |
| mask areas (px) | 2477, 929, 620, 1278, 323 |
| max pairwise IoU | 0.6028 |
| all masks well-formed | yes |
| conversion | 5 instance(s) → 5 measured + 1 interpretive descriptor(s), 4 naming withheld, 0 dropped |
| two-status preserved | yes |
| invariants held | yes |

## Attribution

**organ_succeeded** — the organ measured 5 instance(s) of 'face'. Whether those instances ARE that concept is not established here
