# Review — `sam3-fold-phrase-matrix/veiled_funerary/actuator_direct__face`

*veiled_funerary · 'face' · actuator_direct · mode `actuator_direct` · locked to `concept_segment` · expected `positive`*

> Matrix cell from the pre-registered suite 'sam3-fold-phrase-matrix': family 'availability_control' (role 'availability_control') against fixture 'veiled_funerary'. The phrase was frozen before any live call in this lane and may not be changed now.
...
| lock held | yes |
| cold / warm | warm |
| latency | 6088 ms (load 0 ms) |
| organ status | **ok** |
| instances | 1 |
| mask areas (px) | 63327 |
| max pairwise IoU | — |
| all masks well-formed | yes |
| conversion | 1 instance(s) → 1 measured + 1 interpretive descriptor(s), 0 naming withheld, 0 dropped |
| two-status preserved | yes |
| invariants held | yes |

## Attribution

**organ_succeeded** — the organ measured 1 instance(s) of 'face'. Whether those instances ARE that concept is not established here
