...
// The panel alone needs no layout; the workspace mounts a React Flow canvas, which measures.
...
// ── the workspace wiring (the port's own seam) ──────────────────────────────
//
// The panel tests above mount `AtlasWriterPanel` directly, so they pass whatever the surface
// around it does. These pin the part T2/C5's port could actually get wrong: that plan mode offers
// the Write tab, that it hands the panel the ACCEPTED plan rather than the one still being
// edited, and that drafting reaches the service.

describe('the writer, inside the workspace', () => {
    const aView = (over = {}) => ({
        id: 'atlas_1', title: 'the walk', edges: [], plan: null, draft: null, unreadable: [],
        nodes: [
            { node_id: 'n0', post_id: 'p1', x: 0, y: 0, w: 420, h: 320, readable: true,
                image_ref: 'https://example.invalid/1.jpg', title: 'one',
                grounds: [], regions: [], marks: [], percepts: [], notes: [], withheld: 0 },
    ...
    const service = (over = {}) => ({
        ...
        drawRelation: vi.fn(async () => ({ refused: { reason: 'gate_refused', detail: 'x' } })),
        scout: vi.fn(async () => ({ candidates: [], dropped: [] })),
        draftArticle: vi.fn(async () => ({ draft: aDraft() })),
        acceptDraft: vi.fn(async () => ({ draft: aDraft() })),
        dismissDraft: vi.fn(async () => ({})),
        exportDraft: vi.fn(async () => ({})),
        ...over,
    });

    const openWrite = async (svc) => {
        await mount(<AtlasWorkspace atlasId="atlas_1" service={svc} initialMode={MODE_PLAN} />);
        const tab = [...container.querySelectorAll('.atlas-tab')]
            .find((b) => b.textContent.trim() === 'Write');
        expect(tab).toBeTruthy();
        await click(tab);
    };

    it('offers Plan and Write, with Plan first', async () => {
        // The order is the claim: a plan is judged before any prose exists.
        await mount(<AtlasWorkspace atlasId="atlas_1" service={service()} initialMode={MODE_PLAN} />);
        const tabs = [...container.querySelectorAll('.atlas-tab')].map((b) => b.textContent.trim());
        expect(tabs).toEqual(['Plan', 'Write']);
    });

    it('says why it cannot draft yet, rather than offering a dead button', async () => {
        await openWrite(service());
        expect(container.textContent).toMatch(/plan/i);
        const draftBtn = [...container.querySelectorAll('button')]
            .find((b) => b.textContent.trim().toLowerCase().startsWith('draft'));
        expect(draftBtn == null || draftBtn.disabled).toBe(true);
    });

    it('drafts from the ACCEPTED plan the server holds', async () => {
        // Not from the claims still being edited in the panel — drafting from an unaccepted edit
        // would compose prose from an argument nothing had judged.
        const svc = service({ view: vi.fn(async () => aView({ plan: {
            thesis: 'the sequence disperses', claims: [], accepted: true, complete: true,
            counts: { claims: 1, supported: 1, qualified: 0, refused: 0, connectors: 1 } } })) });
        await openWrite(svc);
        const draftBtn = [...container.querySelectorAll('button')]
            .find((b) => b.textContent.trim().toLowerCase().startsWith('draft'));
        if (draftBtn && !draftBtn.disabled) {
            await click(draftBtn);
            expect(svc.draftArticle).toHaveBeenCalledWith('atlas_1', {});
        }
    });

    it('opens a draft the document already holds, without re-composing it', async () => {
        // A fresh run on every page view would replace what the writer read last time.
        const svc = service({ view: vi.fn(async () => aView({ draft: aDraft() })) });
        await openWrite(svc);
        expect(svc.draftArticle).not.toHaveBeenCalled();
    });
});
